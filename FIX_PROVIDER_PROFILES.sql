...
-- ── 1. Backfill existing users who completed onboarding ──────────────────────

INSERT INTO public.coach_profiles (user_id)
SELECT p.user_id
FROM public.profiles p
WHERE p.role IN ('coach', 'creator')
  AND p.onboarding_completed = true
  AND NOT EXISTS (
    SELECT 1 FROM public.coach_profiles cp WHERE cp.user_id = p.user_id
  )
...
WHERE p.role = 'therapist'
  AND p.onboarding_completed = true
  AND NOT EXISTS (
    SELECT 1 FROM public.therapist_profiles tp WHERE tp.user_id = p.user_id
  )
ON CONFLICT (user_id) DO NOTHING;

-- ── 2. Rebuild complete_onboarding RPC to auto-create secondary rows ──────────
...
BEGIN
  ...
  -- Auto-create secondary provider profile row (coach/creator use coach_profiles)
  INSERT INTO public.coach_profiles (user_id)
  SELECT v_user_id
  WHERE v_role IN ('coach'::app_role, 'creator'::app_role)
  ON CONFLICT (user_id) DO NOTHING;

  INSERT INTO public.therapist_profiles (user_id)
  SELECT v_user_id
  WHERE v_role = 'therapist'::app_role
  ON CONFLICT (user_id) DO NOTHING;
...
-- ── 3. Verify ─────────────────────────────────────────────────────────────────
SELECT 'coach_profiles rows' AS label, COUNT(*)::text AS count FROM public.coach_profiles
UNION ALL
SELECT 'therapist_profiles rows', COUNT(*)::text FROM public.therapist_profiles;
