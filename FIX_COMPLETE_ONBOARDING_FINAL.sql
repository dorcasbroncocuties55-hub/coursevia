-- ============================================================
-- FIX_COMPLETE_ONBOARDING_FINAL.sql
-- Run this once in Supabase SQL Editor.
--
-- Fixes:
--   1. Adds all missing columns to the profiles table
--   2. Drops all existing complete_onboarding overloads
...
ALTER TABLE public.profiles
  ...
  ADD COLUMN IF NOT EXISTS service_delivery_mode   text DEFAULT 'online',
  ADD COLUMN IF NOT EXISTS calendar_mode           text DEFAULT 'provider_calendar',
  ADD COLUMN IF NOT EXISTS meeting_preference      text,
  ADD COLUMN IF NOT EXISTS office_address          text,
  ADD COLUMN IF NOT EXISTS enable_phone_release    boolean DEFAULT true,
  ...
  ADD COLUMN IF NOT EXISTS learner_interests       text,
...
-- ── 2. Drop ALL existing overloads of complete_onboarding ────────────────
...
-- ── 3. Re-create with the exact params the frontend sends ─────────────────
--
-- NOTE: p_languages is text[] to match the JS array the frontend sends.
--       All other params match the rpcPayload in Onboarding.tsx exactly.
...
BEGIN
  ...
  -- Safely cast text → app_role enum
  BEGIN
  ...
  -- Ensure role record exists in user_roles
...
-- Authenticated users can fully manage their own row
...
-- Public (unauthenticated) can read any completed provider profile
...
SELECT 'FIX_COMPLETE_ONBOARDING_FINAL applied successfully' AS result;
