...
const TherapistProfile = () => {
    ...
    const load = async () => {
      ...
      if (p) {
        ...
        setForm({
          ...
          headline: p.headline || t?.headline || "",
          ...
          country: p.country || "",
          city: p.city || "",
          skills: Array.isArray(p.skills) ? p.skills.join(", ") : (Array.isArray(t?.skills) ? t.skills.join(", ") : ""),
          languages: Array.isArray(p.languages) ? p.languages.join(", ") : (Array.isArray(t?.languages) ? t.languages.join(", ") : ""),
          bookingPrice: String(p.booking_price ?? ""),
          hourlyRate: String(t?.hourly_rate ?? p?.hourly_rate ?? ""),
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const slug = slugify(form.profileSlug || form.displayName || form.fullName || `therapist-${user.id.slice(0, 8)}`);
      ...
      const { error: pe } = await supabase.from("profiles").upsert({
        user_id: user.id, role: "therapist", provider_type: "therapist",
      ...
      const { error: te } = await (supabase as any).from("therapist_profiles").upsert({
  ...
  return (
    <DashboardLayout role="therapist">
      <div className="space-y-6 max-w-4xl">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Therapist Profile</h1>
          <p className="mt-1 text-sm text-slate-500">
            This is your public profile. Keep it accurate — clients see this before they book a session with you.
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="flex flex-col gap-6 sm:flex-row sm:items-start">
            ...
            <div className="grid flex-1 gap-4 sm:grid-cols-2">
              <div><Label>Full Name *</Label><Input value={form.fullName} onChange={set("fullName")} placeholder="Dr. Jane Smith" /></div>
              <div><Label>Display Name</Label><Input value={form.displayName} onChange={set("displayName")} placeholder="Dr. Jane" /></div>
              ...
              <div><Label>City</Label><Input value={form.city} onChange={set("city")} placeholder="Abuja" /></div>
              <div><Label>Profile Slug</Label><Input value={form.profileSlug} onChange={set("profileSlug")} placeholder="dr-jane-therapist" /></div>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <div><Label>Headline *</Label><Input value={form.headline} onChange={set("headline")} placeholder="Licensed Clinical Psychologist" /></div>
            <div><Label>Profession</Label><Input value={form.profession} onChange={set("profession")} placeholder="Clinical Psychologist" /></div>
            <div><Label>Qualifications</Label><Input value={form.certification} onChange={set("certification")} placeholder="DipAC, Certificate in Counseling, BSc Psychology" /></div>
            <div><Label>Session Rate (USD)</Label><Input type="number" min="0" value={form.hourlyRate} onChange={set("hourlyRate")} placeholder="60" /></div>
            ...
            <div><Label>Specialties / Skills</Label><Input value={form.skills} onChange={set("skills")} placeholder="Anxiety, Trauma, CBT, Relationships" /></div>
            <div><Label>Languages</Label><Input value={form.languages} onChange={set("languages")} placeholder="English, Igbo, Yoruba" /></div>
            <div>
              <Label>Session Delivery</Label>
            ...
            <div className="sm:col-span-2"><Label>Therapeutic Approach</Label><Textarea rows={4} value={form.experience} onChange={set("experience")} placeholder="Describe your therapeutic approach and methodology…" className="resize-none" /></div>
            <div className="sm:col-span-2"><Label>About Me *</Label><Textarea rows={5} value={form.bio} onChange={set("bio")} placeholder="Write a professional bio that clients will read before booking a session…" className="resize-none" /></div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <p className="mb-5 text-xs text-slate-400">These fields appear on your public therapist profile page.</p>
          <div className="grid gap-4">
            <div>
              <Label>Services Offered <span className="text-xs text-slate-400">(comma separated)</span></Label>
              <Textarea rows={3} value={form.servicesOffered} onChange={set("servicesOffered")} placeholder="Online Therapy, Individual Therapy, Free Consultation, Counselling" className="resize-none" />
            </div>
            <div>
              <Label>Who You Work With <span className="text-xs text-slate-400">(comma separated)</span></Label>
              <Textarea rows={3} value={form.worksWith} onChange={set("worksWith")} placeholder="Adults, Adolescents, Couples, Individuals" className="resize-none" />
            </div>
            <div>
              <Label>Service Areas <span className="text-xs text-slate-400">(comma separated)</span></Label>
              <Input value={form.serviceAreas} onChange={set("serviceAreas")} placeholder="Abuja, Lagos, Ibadan, Online" />
            </div>
          </div>
        </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Verified therapists get a badge on their public profile and can withdraw earnings. Verification is handled securely by our KYC provider.
              </p>
