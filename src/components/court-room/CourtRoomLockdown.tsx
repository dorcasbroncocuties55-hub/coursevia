 ...
 * CourtRoomLockdown
 *
 * Full-portal blocking overlay shown to a coach or therapist when they have
 * an active court case.  Renders the entire court-room experience inline so
 * the provider never needs to navigate away:
 *   - Case header  (case number, disputed amount, dispute type)
 *   - Participants (judge name + rank, learner name, provider name)
 *   - Live chat    (real-time via Supabase channel)
 *   - Evidence     (upload + gallery — both sides can submit)
 *   - Mercy banner (shown when mercy window is active with countdown)
 *
 * Props
 *   userId          – current user's auth uid
 *   role            – "coach" | "therapist"
 *   onMercyWindow   – called when mercy window activates (portal can re-render)
 */
...
import {
  ...
  Music, File, X, CheckCircle, AlertTriangle, Shield, Clock,
  RefreshCw,
} from "lucide-react";
...
const ALLOWED_MIME: Record<string, { label: string }> = {
...
export default function CourtRoomLockdown({ userId, role, onMercyWindow }: Props) {
  // ── Case data ──────────────────────────────────────────────────────────────
  ...
  const [mercyEnd, setMercyEnd] = useState<Date | null>(null);
  const [mercySecondsLeft, setMercySecondsLeft] = useState(0);
  const mercyTimerRef = useRef<ReturnType<typeof setInterval> | null>(null);

  // ── Judge-granted access ───────────────────────────────────────────────────
  const [judgeAccessActive, setJudgeAccessActive] = useState(false);
  const [judgeAccessEnd, setJudgeAccessEnd] = useState<Date | null>(null);
  const [judgeAccessReason, setJudgeAccessReason] = useState<string | null>(null);
  const [judgeAccessSeconds, setJudgeAccessSeconds] = useState(0);
  const judgeAccessTimerRef = useRef<ReturnType<typeof setInterval> | null>(null);
  ...
  // ── Load active court case for this provider ───────────────────────────────
  const loadCase = useCallback(async () => {
    setLoadingCase(true);
    setCaseError(null);
    try {
      // Find open/under_review case where this user is the provider
      ...
      if (!cases || cases.length === 0) {
        // No active case — provider is free
        setCaseData(null);
      ...
      setCaseData({
        ...
        learner_name: (learnerProfile as any)?.full_name || "Learner",
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`lockdown-msgs-${caseData.id}`)
      .on("postgres_changes", {
        event: "INSERT",
        schema: "public",
        table: "case_messages",
  ...
  // ── Mercy window polling (every 60 s) ─────────────────────────────────────
  const checkMercy = useCallback(async () => {
    if (!userId) return;
    try {
      const res = await fetch(`/api/court/provider/restrictions/${userId}`, {
        headers: { "x-user-id": userId },
      });
      const data = await res.json();

      // Mercy window (booking-based)
      if (data?.mercyWindow?.hasAccess) {
        const endTime = data.mercyWindow.accessEnd
          ? new Date(data.mercyWindow.accessEnd)
          : null;
        setMercyActive(true);
        setMercyEnd(endTime);
        onMercyWindow?.();
      ...
      // Judge-granted access
      if (data?.judgeGrantedAccess?.hasAccess) {
        const expiresAt = data.judgeGrantedAccess.expiresAt
          ? new Date(data.judgeGrantedAccess.expiresAt)
          : null;
        setJudgeAccessActive(true);
        setJudgeAccessEnd(expiresAt);
        setJudgeAccessReason(data.judgeGrantedAccess.reason || null);
        onMercyWindow?.();
      } else {
        setJudgeAccessActive(false);
        setJudgeAccessEnd(null);
        setJudgeAccessReason(null);
      }
    } catch {
      // silently ignore
    }
  }, [userId, onMercyWindow]);

  useEffect(() => {
    checkMercy();
    const poll = setInterval(checkMercy, 60_000);
    return () => clearInterval(poll);
  }, [checkMercy]);

  // ── Mercy countdown ────────────────────────────────────────────────────────
  useEffect(() => {
    if (mercyTimerRef.current) clearInterval(mercyTimerRef.current);
    if (!mercyActive || !mercyEnd) return;
    const tick = () => {
      const left = Math.max(0, Math.round((mercyEnd.getTime() - Date.now()) / 1000));
      setMercySecondsLeft(left);
      if (left === 0) { setMercyActive(false); setMercyEnd(null); }
    };
    tick();
    mercyTimerRef.current = setInterval(tick, 1000);
    return () => { if (mercyTimerRef.current) clearInterval(mercyTimerRef.current); };
  }, [mercyActive, mercyEnd]);

  // ── Judge-access countdown ─────────────────────────────────────────────────
  useEffect(() => {
    if (judgeAccessTimerRef.current) clearInterval(judgeAccessTimerRef.current);
    if (!judgeAccessActive || !judgeAccessEnd) return;
    const tick = () => {
      const left = Math.max(0, Math.round((judgeAccessEnd.getTime() - Date.now()) / 1000));
      setJudgeAccessSeconds(left);
      if (left === 0) {
        setJudgeAccessActive(false);
        setJudgeAccessEnd(null);
        setJudgeAccessReason(null);
    ...
    tick();
    judgeAccessTimerRef.current = setInterval(tick, 1000);
    return () => { if (judgeAccessTimerRef.current) clearInterval(judgeAccessTimerRef.current); };
  }, [judgeAccessActive, judgeAccessEnd]);
  ...
  const uploadEvidence = async () => {
    ...
    for (const fu of files) {
      ...
      try {
        ...
        const path = `${caseData.id}/${Date.now()}-${fu.localId}.${ext}`;
  ...
  const senderLabel = (msg: CaseMessage) => {
    ...
    if (msg.sender_id === caseData?.learner_id) return caseData?.learner_name || "Learner";
    if (msg.sender_id === caseData?.provider_id) return caseData?.provider_name || "Provider";
    return "Participant";
  };

  const bubbleColor = (msg: CaseMessage) => {
    if (msg.sender_type === "system") return "bg-blue-900 border border-blue-700 mx-auto text-center text-xs";
  ...
  // No active case — render nothing (parent renders normally)
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-900 flex flex-col overflow-hidden">
      ...
      {/* ── Judge-granted access banner ── */}
      {judgeAccessActive && (
        <div className="bg-blue-600 text-white px-6 py-2 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2 font-semibold text-sm">
            <Gavel size={16} />
            Judge has granted you temporary portal access to gather evidence
            {judgeAccessReason && <span className="font-normal opacity-80">— {judgeAccessReason}</span>}
          </div>
          <span className="font-mono font-bold text-sm bg-blue-700 px-3 py-1 rounded-lg">
            {fmtCountdown(judgeAccessSeconds)} remaining
          </span>
        </div>
      )}

      {/* ── Mercy window banner ── */}
      {mercyActive && !judgeAccessActive && (
        <div className="bg-amber-500 text-amber-900 px-6 py-2 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-2 font-semibold text-sm">
            <Clock size={16} />
            Mercy Window Active — temporary dashboard access granted for your upcoming session
          </div>
          <span className="font-mono font-bold text-sm">
            Closes in {fmtCountdown(mercySecondsLeft)}
          </span>
        </div>
      )}
      ...
      <header className="bg-gray-800 border-b border-gray-700 px-6 py-4 shrink-0">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-3 mt-1 text-sm text-gray-300">
                ...
                <span>•</span>
                <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${caseData.priority_level === "urgent" ? "bg-red-600 text-white" :
          ...
          <div className="flex items-center gap-2">
            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${caseData.status === "resolved" ? "bg-green-700 text-green-100" :
        ...
        <div className="flex gap-8">
          {(["chat", "evidence"] as const).map(tab => (
            <button
              ...
              className={`py-3 text-sm font-medium border-b-2 transition capitalize ${activeTab === tab
      ...
      <div className="flex-1 flex overflow-hidden">
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-3">
                {messages.length === 0 && (
                  <p className="text-center text-gray-500 text-sm mt-8">
                    No messages yet. State your position clearly for the judge.
                  </p>
                )}
                {messages.map(msg => (
              ...
              <div className="border-t border-gray-700 p-4 shrink-0">
                <div className="flex gap-3">
                  <input
                    ...
                    placeholder="State your position for the judge…"
                ...
                <p className="text-xs text-gray-500 mt-1">All messages are visible to the learner and assigned judge.</p>
              </div>
            </>
          )}
            ...
            <div className="flex-1 overflow-y-auto p-4">
              <div className="max-w-2xl mx-auto space-y-5">
                <h3 className="text-lg font-bold text-white flex items-center gap-2">
                  <Upload size={20} className="text-[#0b7e84]" />
                  Submit Evidence
                </h3>
                ...
                <div className="space-y-3">
                  ...
                  <select
                    value={evWeight}
                    onChange={e => setEvWeight(e.target.value as any)}
                    className="w-full bg-gray-700 border border-gray-600 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[#0b7e84]"
                  >
                ...
                <div
                  ...
                  className={`relative border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition ${dragActive ? "border-[#0b7e84] bg-gray-700" : "border-gray-600 hover:border-gray-500"
                    }`}
                >
                  <input
                    ref={fileInputRef}
                ...
                >
                  {uploading ? <><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white" /> Uploading…</> : <><Upload size={16} /> Submit Evidence</>}
                </button>
              </div>
            </div>
          )}
        ...
        <div className="w-72 bg-gray-800 border-l border-gray-700 p-5 flex flex-col gap-4 overflow-y-auto shrink-0 hidden lg:flex">
          ...
          <div className="bg-green-900/60 border border-green-700 rounded-xl p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-green-400 font-medium uppercase tracking-wide">Complainant</p>
          ...
          <div className="bg-purple-900/60 border border-purple-700 rounded-xl p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-xs text-purple-400 font-medium uppercase tracking-wide">Defendant (You)</p>
          ...
          <div className="border-t border-gray-700 pt-4">
            {judgeAccessActive ? (
              <div className="rounded-xl p-3 text-xs space-y-1 bg-blue-900/60 border border-blue-700">
                <p className="font-semibold text-blue-300 flex items-center gap-1">
                  <Gavel size={12} /> Judge-Granted Access Active
                </p>
                <p className="text-blue-200">
                  You have temporary portal access to gather evidence. Closes in {fmtCountdown(judgeAccessSeconds)}.
                </p>
                {judgeAccessReason && (
                  <p className="text-blue-400 italic">"{judgeAccessReason}"</p>
                )}
              </div>
            ) : mercyActive ? (
              <div className="rounded-xl p-3 text-xs space-y-1 bg-amber-900/60 border border-amber-700">
                <p className="font-semibold text-amber-300">⏱ Mercy Window Active</p>
                <p className="text-amber-200">
                  Temporary access for your upcoming session. Closes in {fmtCountdown(mercySecondsLeft)}.
                </p>
              </div>
            ) : (
              <div className="rounded-xl p-3 text-xs space-y-1 bg-gray-700/50 border border-gray-600">
                <p className="font-semibold text-gray-400">🔒 Portal Restricted</p>
                <p className="text-gray-500">
                  Your portal is locked while this case is open. You'll receive an email 30 minutes before
                  each scheduled session, or if the judge grants you evidence-gathering access.
                </p>
              </div>
            )}
          </div>
